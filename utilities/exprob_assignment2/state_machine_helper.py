...
from exprob_assignment2.msg import Point, PlanAction, PlanGoal, ControlAction, ControlGoal
...
class Helper:
	...
	def __init__(self):
		...
		self._room = []                        # Room object
		self._room_coord = dict()              # Dictionary to associate the coordinates X and Y to each room 
		self._connected_to = []                # Rooms connected with the room object
		self._through_door = []                # Doors for connection
		self._rooms = []                       # List of all rooms for debugging purposes
		self._doors = []                       # List of all doors for debugging purposes
	...
	def handle_world_init(self, request):
		...
		"""
		response = WorldInitResponse()  # initialize the service response
		self._room = request.room
		self._room_coord[self._room] = (request.x, request.y)
		self._connected_to = request.connections.connected_to
		self._through_door = request.connections.through_door
		log_msg = f'ROOM: {self._room}'
		rospy.loginfo(anm.tag_log(log_msg, LOG_TAG))
		log_msg = f'Coordinates: ({self._x_coord},{self._y_coord})'
		rospy.loginfo(anm.tag_log(log_msg, LOG_TAG))
		for con in range(0, len(request.connections)):
			log_msg = f'Connected to: {self._connected_to}'
			rospy.loginfo(anm.tag_log(log_msg, LOG_TAG))
			log_msg = f'Through door: {self._through_door}'
			rospy.loginfo(anm.tag_log(log_msg, LOG_TAG))
			ARGS = ['hasDoor', self._room, self._through_door[con]]
			ontology_manager('ADD', 'OBJECTPROP', 'IND', ARGS)
			ARGS = ['hasDoor', self._connected_to[con], self._through_door[con]]
			ontology_manager('ADD', 'OBJECTPROP', 'IND', ARGS)
		if self._room not in self._rooms:
			self._rooms.append(self._room)
		if self._through_door not in self._doors:
			self._doors.append(self._through_door)
		self.markers_detected = self.markers_detected + 1
		if self.markers_detected == 7:
			self.acuto_detected = True
